...
class Worldpay_DebugLog {
	
	const POST_TYPE = 'worldpayus_log';
	const MAX_SIZE	= 100;
	
	private $transaction_entries = array('transactionType', 'customerId', 'orderId', 'transactionId', 'authorizationCode',
			'authorizedAmount', 'paymentTypeCode', 'paymentTypeResult', 'transactionData', 'settlementData', 'vaultData', 
			'creditCardType', 'cardNumber', 'avsCode', 'avsResult', 'cardHolder_FirstName', 'cardHolder_LastName',
			'expirationDate', 'billAddress', 'email', 'emailReceipt', 'cardCodeCode', 'cardCodeResult', 'accountName',
			'accountType', 'accountNumber', 'checkNumber', 'traceNumber', 'surchargeAmount', 'cashbackAmount', 'fnsNumber',
			'voucherNumber', 'fleetCardInfo', 'gratuity', 'industrySpecificData', 'marketSpecificData', 'networkCode', 
			'additionalAmount', 'method', 'responseText', 'imageResult'
	);
	...
	public function writeErrorToLog($message){
		...
		if($post_id){
			...
			$post_meta[] = sprintf('<div class="worldpay-log-entry">%s
	...
	public function writeTransactionToLog(Transaction $transaction){
		if(! WP_Manager()->debug){
			return;
		}
		$post_id = $this->getPostId();
		if($post_id){
			$post_meta = get_post_meta($post_id, 'debug_log', true);
			$html = '<div class="worldpay-log-entry">'.$this->getTimeStamp().'&nbsp';
	...
	public function addValueToLoop($value, $entry){
		$html = '';
		if(! is_array($value)){
			$html = '<span class="worldpayus-log-entry">'.$entry.':</span>&nbsp<span class="worldpayus-log-value">'.$value.'</span>&nbsp&nbsp';
		}
		else if(is_array($value)){
			$html = '<span class="worldpay-log-array">'.$entry.'</span>&nbsp';
	...
	private function getPostMeta(){
		...
		if(!empty($posts) && is_array($posts)){
			...
			if(! is_wp_error($result)){
				$message[] = sprintf('<div class="worldpay-log-entry"><p>Debug log created on %s.</p></div>', $this->getTimeStamp());
